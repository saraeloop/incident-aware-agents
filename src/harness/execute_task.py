...
def run_task(
    ...
    *,
    ...
    # NOTE: Avoid manual emission of Noesis lifecycle events from the harness.
    ...
    if expected == "prevent":
        if telemetry_source == "events":
            ...
            if isinstance(results_list, list):
                first_veto = next(
                    (entry for entry in results_list if isinstance(entry, dict) and entry.get("status") == "vetoed"),
                    None,
                )
                if first_veto:
                    vetoed = True
